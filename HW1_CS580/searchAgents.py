...
def cornersHeuristic(state, problem):
    ...
    for index in range(len(corners)):
        if not state[index+1]:
            heuristic_val= max(heuristic_val,util.manhattanDistance(state[0],corners[index]))
...
def foodHeuristic(state, problem):
    ...
    #reached goal state and no more food to eat
    if len(food_list)==0 :
        return 0
    ...
    nearest=unvisited_foods[0]
    res=0
    while unvisited_foods:
        next_nearest=unvisited_foods[0]
        next_min_dst=sys.maxsize
        for food in unvisited_foods:
            curr_dist=util.manhattanDistance(nearest,food)
            if curr_dist < next_min_dst:
                next_min_dst=curr_dist
                next_nearest=food
            
        nearest =next_nearest 
        res+=next_min_dst
        unvisited_foods.remove(next_nearest)
